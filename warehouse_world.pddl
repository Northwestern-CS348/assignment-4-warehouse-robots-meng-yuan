(define (domain warehouse)
   ...
   (:action robotMove
      ...
      :effect (and  (at ?r ?l2) (not (at ?r ?l2)) (not (no-robot ?l2)) (no-robot ?l1) )

   )


   (:action robotMoveWithPallette
      :parameters (?l1 ?l2 - location ?r - robot ?p - pallette)
   ...
   (:action moveItemFromPalletteToShipment
      ...
      :precondition (and  (ships ?s ?o) (orders ?o ?si) (packing-location ?l) (contains ?p ?si ) (at ?p ?l) )
   ...
   (:action completeShipment
      :parameters ( ?s - shipment ?o - order ?l - location)
      :precondition (and  (not( complete ?s)) (packing-at ?s ?l) (ships ?s ?o))
      :effect (and (not(started ?s)) (complete ?s) (not(packing-at ?s ?l)) (not (unstarted ?s)) )
   )
